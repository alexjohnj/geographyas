# PorterStemmer1980.js rewritten in CoffeeScript. I have absolutely no idea what's going on in this. The original comments are below. 
# - @alexjohnj
...
stemmer = (w, debug) ->
  ...
  dummyDebug = -> return null
  realDebug = -> console.log Array.prototype.slice.call(arguments).join(' ')

  debugFunction = if debug then realDebug else dummyDebug
  origword = w
  ...
  if re.test w
    ...
    if re.test stem or re2.test stem and not re3.test stem
      w = stem
